# EIMAS_NH — NH Trading Intelligence Copilot

**NH투자증권 Trading 지원용 얇은 rates/FICC memo 프로젝트: 금리, 채권, 커브, 실행 리스크를 메모 형태로 구조화**
...
`forecast`의 확률/과신 문제와 `EIMAS`의 구조화 능력을 묶어,
**금리와 채권 시장 이벤트를 NH rates/FICC desk가 읽을 수 있는 risk-aware memo**로 바꾸는 파생 프로젝트.

## Core Message

이 프로젝트가 보여주려는 사람상은 아래 한 줄이다.

**저는 데이터를 앞세우는 사람이 아니라, 금리와 채권 시장 이벤트를 해석하고 그 판단 근거를 통계와 데이터로 검증해 NH rates/FICC desk가 바로 읽을 수 있는 메모로 구조화하는 사람입니다.**

핵심 planning 문서:

- [core_message.md](docs/planning/active/nh-trading-intelligence/core_message.md)
- [north_star.md](docs/planning/active/nh-trading-intelligence/north_star.md)
- [plan.md](docs/planning/active/nh-trading-intelligence/plan.md)
- [todo.md](docs/planning/active/nh-trading-intelligence/todo.md)

## Current Vertical

현재 EIMAS_NH는 범용 투자 자동화보다 **NH투자증권 Trading의 채권/금리/FICC 문맥에 직접 연결되는 얇은 memo copilot**로 방향을 좁혔다.

- 기본 profile: `nh-trading-v1`
- 핵심 산출물: `NHTradingDeskMemoV0`
- 기본 원칙: `risk-aware`, `evidence-first`, `human handoff`, `no auto execution`
- 핵심 연구 연계: [paper_v2.md](/home/tj/projects/forecast/phd/paper_v2.md)
...
`nh-trading-v1` 는 NH Trading memo vertical을 위한 baseline이다.
portfolio/trade-plan/backtest는 줄이고, institutional context와 market structure, memo packaging을 우선한다.
...
- [docs/features/EIMAS_V2_DIRECTION.md](docs/features/EIMAS_V2_DIRECTION.md) — v2 포지셔닝 및 리팩토링 기준
- [docs/planning/README.md](docs/planning/README.md) — 긴 문서와 분리된 plan/todo 운영 규칙
- [docs/planning/active/nh-trading-intelligence/plan.md](docs/planning/active/nh-trading-intelligence/plan.md) — 현재 활성 구현 계획
